...
const calculatorMethods = {
  a: {
    ...
    fromBAlpha: (b, alpha) => b * Math.tan((alpha * Math.PI) / 180),

    fromCAlpha: (c, alpha) => c * Math.sin((alpha * Math.PI) / 180),

    fromBBeta: (b, beta) => b / Math.tan((beta * Math.PI) / 180),

    fromCBeta: (c, beta) => c * Math.cos((beta * Math.PI) / 180),
  },
  b: {
    fromAC: (a, c) => Math.sqrt(c * c - a * a),

    fromAAlpha: (a, alpha) => a / Math.tan((alpha * Math.PI) / 180),

    fromCAlpha: (c, alpha) => c * Math.cos((alpha * Math.PI) / 180),

    fromABeta: (a, beta) => a * Math.tan((beta * Math.PI) / 180),

    fromCBeta: (c, beta) => c * Math.sin((beta * Math.PI) / 180),
  },
  c: {
    fromAB: (a, b) => Math.sqrt(a * a + b * b),

    fromAAlpha: (a, alpha) => a / Math.sin((alpha * Math.PI) / 180),

    fromBAlpha: (a, alpha) => b / Math.cos((alpha * Math.PI) / 180),

    fromBBeta: (b, beta) => b / Math.sin((beta * Math.PI) / 180),

    fromABeta: (a, beta) => a / Math.cos((beta * Math.PI) / 180),
  },
  alpha: {
    ...
    fromCBeta: (c, beta) =>
      (Math.asin(c * Math.sin((beta * Math.PI) / 180)) * 180) / Math.PI,
  },
  beta: {
    ...
    fromCAlpha: (c, alpha) =>
      (Math.acos(c * Math.cos((alpha * Math.PI) / 180)) * 180) / Math.PI,
  },
};
...
function calculate(target) {
  ...
  try {
    ...
    switch (target) {
      case "a":
        if (values.b && values.c) {
          ...
          calculated = methods.fromBAlpha(values.b, values.alpha);
        } else if (values.c && values.alpha) {
          calculated = methods.fromCAlpha(values.c, values.alpha);
        } else if (values.b && values.beta) {
          calculated = methods.fromBBeta(values.b, values.beta);
        } else if (values.c && values.beta) {
          calculated = methods.fromCBeta(values.c, values.beta);
        }
        break;
      ...
      case "b":
        if (values.a && values.c) {
          ...
          calculated = methods.fromAAlpha(values.a, values.alpha);
        } else if (values.c && values.alpha) {
          calculated = methods.fromCAlpha(values.c, values.alpha);
        } else if (values.a && values.beta) {
          calculated = methods.fromABeta(values.a, values.beta);
        } else if (values.c && values.beta) {
          calculated = methods.fromCBeta(values.c, values.beta);
        }
        break;
    ...
    if (calculated !== undefined) {
      ...
      const symbol = target === "alpha" || target === "beta" ? "°" : "";
